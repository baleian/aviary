...
from aviary_shared.db.models import Agent, AgentACL, TeamMember, Workflow, WorkflowACL
...
WORKFLOW_ROLE_PERMISSIONS = {
    "viewer": {"search", "view"},
    "user": {"search", "view", "execute"},
    "admin": {"search", "view", "execute", "edit_config"},
    "owner": {"search", "view", "execute", "edit_config", "manage_acl", "delete"},
}


async def resolve_workflow_role(
    db: AsyncSession, user_id: uuid.UUID, workflow: Workflow
) -> str | None:
    """Resolve the effective role a user has on a workflow. Returns None if no access."""
    ...
    if workflow.visibility == "team" and user_team_ids:
        owner_team_ids_result = await db.execute(
            select(TeamMember.team_id).where(TeamMember.user_id == workflow.owner_id)
        )
        owner_team_ids = {row[0] for row in owner_team_ids_result.all()}
        if owner_team_ids & set(user_team_ids):
            return "user"
...
def has_workflow_permission(role: str | None, permission: str) -> bool:
    """Check if a role grants a specific workflow permission."""
    if role is None:
        return False
    return permission in WORKFLOW_ROLE_PERMISSIONS.get(role, set())
