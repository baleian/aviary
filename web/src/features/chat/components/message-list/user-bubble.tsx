...
export function UserBubble({ content, showAvatar = true, targetId, attachments }: UserBubbleProps) {
  ...
  return (
    <div className="flex flex-row-reverse gap-3 group animate-fade-in">
      {showAvatar ? (
        <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-aurora-a type-small text-white shadow-[0_0_12px_rgba(123,92,255,0.4),inset_0_1px_0_rgba(255,255,255,0.2)]">
          You
        </div>
      ) : (
      ...
      <div
        ...
        className={cn(
          "relative max-w-[60%] gradient-border-a rounded-xl rounded-tr-sm transition-shadow",
          isActiveMatch && "ring-2 ring-aurora-violet/60 ring-offset-2 ring-offset-canvas",
        )}
      >
        <div className="rounded-xl rounded-tr-sm bg-aurora-a-soft px-4 py-3 type-body text-fg-primary">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {attachments.map((att) => (
                <button
                  ...
                  className="block rounded-lg overflow-hidden border border-white/10 hover:border-aurora-violet/40 transition-colors cursor-zoom-in"
                >
                  <img
