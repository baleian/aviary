...
**API 서버**는 사용자 대면 기능(인증, 채팅, 에이전트 설정)을 처리합니다. **Admin 콘솔**은 인프라 설정(정책, 배포)을 편집합니다. **Agent Controller**는 K8s 내부에서 런타임 리소스 관리, 오토스케일링, 유휴 정리를 수행합니다. **Egress Proxy**는 에이전트별 아웃바운드 트래픽 규칙을 적용합니다.
...
# K8s 이미지 리빌드 (runtime, egress-proxy, agent-controller)
