...
**API 서버**는 사용자 대면 기능(인증, 채팅, 에이전트 설정)을 처리합니다. **Admin 콘솔**은 인프라 설정(정책, 배포)을 편집합니다. **Agent Supervisor**는 K8s 내부에서 런타임 리소스 관리, 오토스케일링, 유휴 정리를 수행합니다. **Egress Proxy**는 에이전트별 아웃바운드 트래픽 규칙을 적용합니다.
...
- **Inference Router** — 중앙 집중식 LLM 게이트웨이; `X-Backend` 헤더(런타임이 `ANTHROPIC_CUSTOM_HEADERS`를 통해 주입)로 백엔드가 투명하게 라우팅
...
### Inference Router
세션 Pod는 LLM 백엔드를 직접 호출하지 않습니다. 모든 추론은 중앙 라우터를 통하며, 런타임이 `ANTHROPIC_CUSTOM_HEADERS`를 통해 주입하는 `X-Backend` 헤더로 백엔드를 결정합니다 (예: `claude` → Claude API, `ollama` → Ollama). API 자격증명을 중앙 관리하고, Anthropic Messages API를 네이티브로 사용하기 때문에 claude-agent-sdk의 모든 기능이 보존됩니다. API 서버도 모델 목록 조회 시 동일 라우터를 경유하여, 접근 제어의 단일 적용 지점을 보장합니다.
