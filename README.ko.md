...
└───┬───────────┬───────────┬────────────────────────────────────┘
    ...
    │           │   │                                            │
    │           │   │  ┌─────────────────────────────────────┐   │
    │           │   │  │ MCP Gateway (도구 프록시 + 카탈로그) │◀─ Vault
    │           │   │  │  OIDC 인증 · ACL · 도구 디스커버리   │   (도구 자격증명)
    │           │   │  └────────┬────────────────────────────┘   │
    │           │   │           ▼                                 │
    │           │   │     백엔드 MCP 서버                          │
    │           │   └────────────────────────────────────────────┘
    ...
    └─▶│  PostgreSQL    │  │    Redis      │  │   Keycloak     │
       ...
       │  ACL, 에이전트  │  │  presence │  │   팀 동기화     │
       └───────────────┘  └──────────────┘  └────────────────┘
```

**API 서버**는 사용자 대면 기능(인증, 채팅, 에이전트 설정)을 처리합니다. **Admin 콘솔**은 인프라 설정(정책, 배포)을 편집합니다. **Agent Supervisor**는 K8s 내부에서 런타임 리소스 관리, 오토스케일링, 유휴 정리를 수행합니다. **Egress Proxy**는 에이전트별 아웃바운드 트래픽 규칙을 적용합니다. **MCP Gateway**는 중앙 집중식 도구 관리 — 카탈로그, 디스커버리, 유저별 ACL, MCP 도구 호출 프록시를 제공합니다.
...
- **MCP Gateway** — [Model Context Protocol](https://modelcontextprotocol.io/)을 통한 중앙 집중식 도구 관리; 관리자가 MCP 서버를 등록하면 도구가 자동 디스커버리되며, 사용자는 유저별 ACL이 적용된 도구 카탈로그에서 원하는 도구를 에이전트에 바인딩 (기본값: 모두 차단)
- **LiteLLM + Portkey Gateway** — 모든 LLM 호출이 [LiteLLM](https://github.com/BerriAI/litellm)과 [Portkey AI Gateway](https://github.com/portkey-ai/gateway)를 통해 라우팅; LiteLLM이 모델 라우팅과 유저별 API 키 주입을 처리하고, Portkey가 가드레일, OpenTelemetry 트레이싱, 요청 로깅, 캐싱을 제공
- **멀티 백엔드 추론** — Claude API, Ollama, vLLM, AWS Bedrock; 설정 파일로 새 백엔드 추가, 코드 또는 NetworkPolicy 변경 불필요
...
- **Vault 기반 자격증명 주입** — 유저별 Anthropic API 키는 LiteLLM Gateway에서, MCP 도구 자격증명은 MCP Gateway에서 주입; 모든 시크릿은 OIDC 토큰 전파를 통해 Vault에서 조회되며, 에이전트 Pod에 노출되지 않음
...
| 에이전트 런타임 | TypeScript, Node.js 22, claude-agent-sdk, Claude Code CLI |
| LLM 게이트웨이 | [LiteLLM](https://github.com/BerriAI/litellm) + [Portkey](https://github.com/portkey-ai/gateway) — 통합 LLM 프록시 + AI 게이트웨이 (가드레일, 트레이싱, 캐싱) |
| MCP Gateway | Python 3.12, FastAPI, [MCP SDK](https://github.com/modelcontextprotocol/python-sdk) — 도구 카탈로그, ACL, 프록시 |
...
├── mcp-gateway/             # MCP Gateway (FastAPI) — 도구 카탈로그, ACL, 프록시
│   └── app/                 # OIDC 인증, MCP 프로토콜 프록시, 도구 디스커버리
├── agent-supervisor/         # Agent Supervisor (FastAPI, K8s 내부 실행)
│   └── app/                 # K8s 게이트웨이, 에이전트 중심 + K8s 전용 API
├── runtime/                 # 에이전트 런타임 (에이전트 Pod 내부에서 실행)
│   └── src/                 # claude-agent-sdk 하네스, 세션 매니저
├── config/litellm/          # LiteLLM 게이트웨이 설정
├── mcp-servers/             # 플랫폼 제공 MCP 서버 스텁 (GitHub, Jira, Confluence)
...
소스 코드가 컨테이너에 bind mount되어 있어서 `api/`, `web/` 파일 수정이 핫 리로드로 자동 반영됩니다. LiteLLM 설정 변경 시 `docker compose restart litellm`으로 재시작합니다.
...
# K8s 이미지 리빌드 (runtime, egress-proxy, agent-supervisor)
...
API 및 admin 테스트: 헬스, 에이전트 CRUD, ACL (가시성, 권한 부여/거부), 세션 (생성, 목록, 접근 제어, 아카이브), 정책, 배포. 전용 `aviary_test` 데이터베이스와 토큰 기반 mock 인증으로 다중 사용자 시나리오를 지원합니다.
...
| GET | `/api/inference/models` | 전체 백엔드의 사용 가능한 모델 |

### MCP 도구
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/api/mcp/servers` | 사용 가능한 MCP 서버 목록 (ACL 필터링) |
| GET | `/api/mcp/servers/{id}/tools` | 서버의 도구 목록 |
| GET | `/api/mcp/tools/search?q=...` | 이름 또는 설명으로 도구 검색 |
| GET/PUT | `/api/mcp/agents/{id}/tools` | 에이전트의 도구 바인딩 조회 / 설정 |
| DELETE | `/api/mcp/agents/{id}/tools/{tool_id}` | 도구 바인딩 해제 |

## 주요 설계 결정

### LiteLLM + Portkey Gateway
세션 Pod는 LLM 백엔드를 직접 호출하지 않습니다. 모든 추론은 2계층 게이트웨이를 통합니다: [LiteLLM](https://github.com/BerriAI/litellm) OSS 프록시가 모델 이름 접두사로 백엔드를 결정하고 (예: `anthropic/claude-sonnet-4-6` → Claude API, `ollama/gemma4:26b` → Ollama, `vllm/...` → vLLM), [Portkey AI Gateway](https://github.com/portkey-ai/gateway)가 실제 LLM 백엔드로 전달하면서 가드레일, OpenTelemetry 기반 트레이싱, 요청/응답 로깅, 캐싱을 제공합니다. LiteLLM은 Anthropic Messages API (`/v1/messages`)를 네이티브로 지원하므로, claude-agent-sdk가 프로토콜 변환 없이 투명하게 동작합니다. API 자격증명, 속도 제한, 관측성이 중앙 집중화되며, 새 백엔드는 설정 파일(`config/litellm/config.yaml`)로 코드 변경 없이 추가할 수 있습니다.

### MCP Gateway
에이전트 도구 호출은 중앙 집중식 [MCP](https://modelcontextprotocol.io/) Gateway를 통해 라우팅됩니다. claude-agent-sdk 관점에서 게이트웨이는 단일 MCP 서버로 동작합니다. 운영자가 Admin 콘솔을 통해 백엔드 MCP 서버를 등록하면, MCP 프로토콜을 통해 도구가 자동 디스커버리됩니다. 사용자는 ACL이 적용된 도구 카탈로그에서 도구를 선택하여 에이전트에 바인딩합니다. 런타임에서 사용자의 OIDC 토큰이 브라우저부터 API를 거쳐 게이트웨이까지 전파되며, 게이트웨이는 모든 `tools/list` 및 `tools/call` 요청에서 권한을 검증합니다. 사용자 토큰은 외부 서비스로 전달되지 않습니다.

### Egress Proxy
에이전트 Pod의 모든 아웃바운드 HTTP/HTTPS 트래픽은 중앙 포워드 프록시를 통해 라우팅됩니다 (`HTTP_PROXY`/`HTTPS_PROXY` 환경변수). 프록시는 소스 Pod IP를 K8s 네임스페이스로 변환하여 에이전트를 식별하고, 에이전트별 이그레스 정책을 적용합니다. 지원하는 규칙: CIDR 범위, 정확한 도메인, 와일드카드 도메인(`*.example.com`), 전체 허용. 기본값은 모두 차단이며, 정책 변경 시 Pod 재시작 없이 즉시 적용됩니다.
...
### 에이전트 Pod 전략
각 에이전트는 장기 실행 Deployment로 구동되며, 스폰 전략을 설정할 수 있습니다: `lazy` (기본값, 첫 메시지 시 생성) 또는 `eager` (에이전트 생성 시). 여러 세션이 동일 Pod을 공유하며 워크스페이스 디렉토리와 bubblewrap 샌드박스로 격리됩니다. 유휴 에이전트(7일)는 삭제되지 않고 0으로 스케일 다운되며, 다음 메시지에서 재활성화됩니다.
...
| 서비스 | 포트 | 역할 |
|--------|------|------|
| `api` | 8000 | API 서버 (사용자 대면) |
| `admin` | 8001 | Admin 콘솔 (운영자 대면, 인증 없음) |
| `web` | 3000 | Web UI |
| `litellm` | 8090 | LLM 게이트웨이 (LiteLLM + Portkey) |
| `mcp-gateway` | 8100 | MCP 도구 프록시, 카탈로그, ACL |
