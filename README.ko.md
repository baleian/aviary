...
- **LiteLLM Gateway** — 모든 LLM 호출이 [LiteLLM](https://github.com/BerriAI/litellm) OSS 프록시를 통해 라우팅; 모델 이름 접두사(`anthropic/`, `ollama/`, `vllm/`, `bedrock/`)로 백엔드 결정, Anthropic SDK와 네이티브 호환
...
| LLM 게이트웨이 | [LiteLLM](https://github.com/BerriAI/litellm) — 통합 LLM 프록시 (Anthropic, OpenAI, Bedrock, Ollama, vLLM) |
...
### LiteLLM Gateway
세션 Pod는 LLM 백엔드를 직접 호출하지 않습니다. 모든 추론은 [LiteLLM](https://github.com/BerriAI/litellm) OSS 프록시를 통하며, 모델 이름 접두사로 백엔드를 결정합니다 (예: `anthropic/claude-sonnet-4-6` → Claude API, `ollama/gemma4:26b` → Ollama, `vllm/...` → vLLM). LiteLLM은 Anthropic Messages API (`/v1/messages`)를 네이티브로 지원하므로, claude-agent-sdk가 프로토콜 변환 없이 투명하게 동작합니다. API 자격증명, 속도 제한, 관측성이 중앙 집중화되며, 새 백엔드는 설정 파일(`config/litellm/config.yaml`)로 코드 변경 없이 추가할 수 있습니다.
...
| `web` | 3000 | Web UI |
| `litellm` | 8090 | LLM 게이트웨이 (LiteLLM) |
