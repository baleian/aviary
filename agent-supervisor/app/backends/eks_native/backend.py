"""EKS Native RuntimeBackend — managed-node EKS with VPC CNI per-pod SGs.

Assumes the supervisor runs inside the EKS cluster (IRSA or node role grants
Kubernetes API access). Storage is EFS via the EFS CSI driver; egress is
controlled via AWS security groups attached via `SecurityGroupPolicy`.
"""
...
from app.backends.eks_native.manifests import (
...
class EKSNativeBackend(RuntimeBackend):
    ...
    async def register_agent(self, spec: AgentSpec) -> None:
        ...
        # Baseline SGs always applied — even before an agent has extras bound.
