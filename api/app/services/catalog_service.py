...
async def compute_facets(
    db: AsyncSession, *, q: str | None, categories: list[str] | None
) -> dict:
    """Facet counts over the live (published) catalog.

    `q` narrows to name/description trigram; `categories` narrows the server
    facet only (categories facet itself is always unfiltered by category, so
    toggling one doesn't zero the others)."""

    base = (
        select(AgentVersion, CatalogAgent)
        .select_from(CatalogAgent)
    )
    base = _live_join(base)
    ...
    # Category facet (unaffected by category selection).
    cat_stmt = (
        select(AgentVersion.category, func.count(CatalogAgent.id))
        .select_from(base.subquery().alias("b"))
        .group_by(AgentVersion.category)
    )
    # We can't reuse base_subquery with joined columns cleanly — redo explicitly.
    cat_live = (
        select(AgentVersion.category, func.count(CatalogAgent.id))
        .select_from(CatalogAgent)
        .join(AgentVersion, AgentVersion.id == CatalogAgent.current_version_id)
        .where(
            CatalogAgent.is_published.is_(True),
            CatalogAgent.unpublished_at.is_(None),
            AgentVersion.unpublished_at.is_(None),
    ...
    cat_rows = (await db.execute(cat_live)).all()
    categories_facet = [
        {"name": name, "count": int(cnt)} for name, cnt in cat_rows
    ]

    # Server facet — extract unique server_name per version, count across the
    # filtered population. We compute this in Python for simplicity.
    srv_base = (
        select(AgentVersion.mcp_tool_bindings)
        .select_from(CatalogAgent)
        .join(AgentVersion, AgentVersion.id == CatalogAgent.current_version_id)
        .where(
            CatalogAgent.is_published.is_(True),
    ...
    if categories:
        srv_base = srv_base.where(AgentVersion.category.in_(categories))

    server_counts: dict[str, int] = {}
    for (bindings,) in (await db.execute(srv_base)).all():
